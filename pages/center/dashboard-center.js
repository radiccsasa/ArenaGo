...
function loadStatistics() {
    $.ajax({
        ...
        success: function (response) {
            console.log("Statistika odgovor:", response);
            if (response.status === "success" && response.data) {
                $("#totalTerms").text(response.data.total_terms || 0);
                $("#totalReservations").text(response.data.total_reservations || 0);
                
                // Ispravljeno: koristimo odgovarajuća imena iz response.data
                $("#reservationDetails").html(
                    `(${response.data.approved || 0} potvrđenih, ${response.data.pending || 0} na čekanju, ${response.data.cancelled || 0} odbijenih)`
                );
            }
        },
        error: function (xhr, status, error) {
...
function loadReservations(status = "", search = "") {
    console.log("loadReservations - status:", status, "search:", search);
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log("loadReservations - odgovor:", response);
            if (response.status === "success") {
        ...
        error: function (xhr, status, error) {
            console.error("loadReservations - greška:", error);
            console.error("loadReservations - odgovor servera:", xhr.responseText);
            $("#reservationsBody").html(
...
function displayTermsGrid(terms) {
    ...
    terms.forEach(function (term) {
        ...
        // Status termina (prošao/danas/predstoji)
        let statusClass = termDate < today ? "bg-warning" : 
                         termDate.toDateString() === today.toDateString() ? "bg-success" : "bg-primary";
        let statusText = termDate < today ? "Prošao" : 
                        termDate.toDateString() === today.toDateString() ? "Danas" : "Predstoji";

        // Provera da li je termin rezervisan (approved)
        // Ovo pretpostavlja da term objekat ima polje is_reserved ili reservation_count
        let isReserved = term.is_reserved || (term.reservation_count && term.reservation_count > 0);
        
        // Ako nema tih polja, možete dodati poseban upit u API da vrati broj approved rezervacija
            ...
            <div class="col">
                <div class="card h-100 shadow-sm">
                    <div class="card-header ${statusClass} text-white d-flex justify-content-between align-items-center">
                        ...
                        ${isReserved ? 
                            '<span class="badge bg-danger"><i class="bi bi-calendar-check"></i> Rezervisan</span>' : 
                            '<span class="badge bg-success"><i class="bi bi-calendar"></i> Slobodan</span>'
                        }
                    </div>
                    <div class="card-body">
                        <h5 class="card-title">
                        ...
                        <div class="mb-2"><i class="bi bi-calendar"></i> Datum: <strong>${formattedDate}</strong></div>
                        <div class="mb-2"><i class="bi bi-clock"></i> Vreme: <strong>${term.time}</strong></div>
                        <div class="mb-2"><i class="bi bi-people"></i> Kapacitet: <strong>${term.capacity}</strong></div>
                        <div class="mb-2">
                ...
                : `<span class="fw-bold">${formatPrice(price)}</span>`
                        ...
                        ${discount > 0 ? `<div class="mb-2"><span class="badge bg-danger">Popust ${discount}%</span></div>` : ""}
                    </div>
                    <div class="card-footer bg-transparent">
                        <div class="d-flex justify-content-between">
                            ...
                            ${termDate < today ? "" : `<a href="/ArenaGo/pages/add-term/add-term.php?id=${term.id}" class="btn btn-sm btn-outline-primary">
...
function displayReservations(reservations) {
    ...
    if (!reservations || reservations.length === 0) {
        html = '<tr><td colspan="9" class="text-center">Nema rezervacija za prikaz</td></tr>';
    } else {
        reservations.forEach(function (res) {
            ...
            switch (res.status) {
                case "pending":
                    statusBadge = '<span class="badge bg-warning text-dark">Na čekanju</span>';
                ...
                case "cancelled": 
                    ...
                    break;
                default:
                    statusBadge = '<span class="badge bg-secondary">' + (res.status || "Nepoznato") + '</span>';
                    statusText = res.status || "Nepoznato";
            }
            ...
            if (res.status === "pending") {
                ...
                html += `<span class="text-muted small">${statusText}</span>`;
            }
...
function displayComments(comments) {
    ...
    comments.forEach(function (comment) {
        ...
        let formattedDate = commentDate.toLocaleDateString("sr-RS") + " " +
            commentDate.toLocaleTimeString("sr-RS", { hour: "2-digit", minute: "2-digit" });
        let hasResponse = comment.comment_response && comment.comment_response.trim() !== "";
                ...
                <div class="card-body">
                    ...
                    ${!hasResponse ? `
                        <div class="mt-3 reply-form-${comment.id}">
                            ...
                            <button class="btn btn-sm btn-success submit-reply" data-id="${comment.id}">
...
function formatDate(dateString) {
    ...
    try {
        ...
        return isNaN(date.getTime()) ? dateString : date.toLocaleDateString("sr-RS");
    } catch (e) {
        return dateString;
    }
}
...
function formatPrice(price) {
    ...
    try {
        return new Intl.NumberFormat("sr-RS", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(price) + " RSD";
    } catch (e) {
        return price + " RSD";
    }
}
function updateReservationStatus(reservationId, status) {
    // status je već 'approved' ili 'cancelled' - nema konverzije
    let actionText = status === "approved" ? "prihvatite" : "odbijete";
    
    if (!confirm(`Da li ste sigurni da želite da ${actionText} ovu rezervaciju?`)) {
        return;
    }
    ...
    $.ajax({
        ...
        data: {
            ...
            status: status, // Šaljemo 'approved' ili 'cancelled' direktno
        },
        success: function (response) {
            if (response.status === "success") {
                showToast(`Rezervacija je uspešno ${status === "approved" ? "prihvaćena" : "odbijena"}!`, "success");
                loadStatistics();
                loadReservations($("#filterStatus").val(), $("#searchReservation").val());
            } else {
                showToast("Greška: " + (response.message || "Nepoznata greška"), "error");
            }
        },
        error: function (xhr, status, error) {
            showToast("Došlo je do greške na serveru: " + error, "error");
